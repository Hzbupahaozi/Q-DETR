...
def Quantnoise(input_weight, ratio, alpha, Qn, Qp):
    device = torch.device("cuda")
    input_weight = input_weight.to(device)
    map = torch.rand(input_weight.shape).to(device)
    mask = map > ratio
    no_round_weight = input_weight - mask * input_weight
    round_weight = input_weight - no_round_weight
    w_q_half = round_pass((round_weight / alpha).clamp(Qn, Qp)) * alpha
    w_q = w_q_half + no_round_weight
    return w_q
...
class Conv2dLSQ(_Conv2dQ):
    ...
    def forward(self, x):
        ...
        # nbits=4，对权重的量化
        Qn = -2 ** (self.nbits - 1)  # Qn=-8
        Qp = 2 ** (self.nbits - 1) - 1  # Qp=7
        ...
        # alpha是scale，对scale的梯度乘以一个缩放因子g
        alpha = grad_scale(self.alpha, g)
        # print(alpha.shape)
        # print(self.weight.shape)
        alpha = alpha.unsqueeze(1).unsqueeze(2).unsqueeze(3)
        w_q = round_pass((self.weight / alpha).clamp(Qn, Qp)) * alpha
        # w_q = Quantnoise(self.weight, 0.2, alpha, Qn, Qp)
...
class ActLSQ(_ActQ):
    ...
    def forward(self, x):
        ...
        if self.training and self.init_state == 0:
            ...
            # print(self.zero_point.data, 0.1 * (torch.min(x.detach()) - self.alpha.data * Qn))
